...
@interface HFOpenGLView () {
    ...
    const GLfloat *_preferredCinversion;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        ...
        _preferredCinversion = kColorConversion709;
        [self setUpView];
    }
    return self;
}
...
- (void)setupGL {
    ...
    glUniformMatrix3fv(uniforms[UNIFORM_COLOR_CONVERSION_MATRIX], 1, GL_FALSE, _preferredCinversion);
    ...
    GLKMatrix4 modelViewMatrix = GLKMatrix4MakeLookAt(0, 0, 0, 1, 0, 0, 0, 1, 0);
...
- (void)setupBuffers {
    ...
    glVertexAttribPointer(ATTRIB_TEXCOORD, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GL_FLOAT), 0);
...
- (BOOL)loadShaders {
    ...
    uniforms[UNIFORM_PROJECTION_MATRIX] = glGetUniformLocation(self.program, "projectionMatrix");
...
- (void)displayWithPixelBuffer:(CVPixelBufferRef)pixelBuffer {
    ...
    if (pixelBuffer != NULL) {
        ...
        if ([EAGLContext currentContext] != _context) {
            [EAGLContext setCurrentContext:_context]; /*  注意：一定要加这行  */
        }
        [self cleanUpTexture];
        ...
        if (colorAttachments == kCVImageBufferYCbCrMatrix_ITU_R_601_4) {
            if (self.isFullYUVRange) {
                _preferredCinversion = kColorConversion601FullRange;
            } else {
                _preferredCinversion = kColorConversion601;
            }
        } else {
            _preferredCinversion = kColorConversion709;
        }
        ...
        if (err) {
            NSLog(@"Eroro at CVOpenGLESTextureCacheCreateTextureFromImage %d",err);
        }
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (HF_ROTATE) {
        [self rotateWithX:point.y - prePoint.y Y:point.x - prePoint.x];
    } else {
...
- (void)rotateWithX:(float)x Y:(float)y {
    ...
    GLKMatrix4 modelViewMtrix = GLKMatrix4Identity;
    modelViewMtrix = GLKMatrix4RotateX(modelViewMtrix, horizontalDegree);
    modelViewMtrix = GLKMatrix4RotateY(modelViewMtrix, verticalDegree);
    
    
    GLKMatrix4 projectionMatrix = GLKMatrix4MakePerspective(90, CGRectGetWidth(self.bounds) * 1.0 / CGRectGetHeight(self.bounds), 0.01, 10);
    
    GLKVector4 position = GLKVector4Make(0, 0, -1, 1);
    GLKVector4 targetPosition = GLKMatrix4MultiplyVector4(GLKMatrix4Multiply(projectionMatrix, modelViewMtrix), position);
    ...
    glUniformMatrix4fv(uniforms[UNIFORM_MODELVIEW_MARTRIX], 1, GL_FALSE, modelViewMtrix.m);
...
- (void)changeModelViewWithHorizontal:(float)h Vertical:(float)v {
    ...
    GLKMatrix4 modelViewMatrix = GLKMatrix4MakeLookAt(0, 0, 0,
                                                      sin(horizontalDegree) * cos(verticalDegree), sin(horizontalDegree) * sin(verticalDegree), cos(horizontalDegree),
                                                      0, 1, 0);
